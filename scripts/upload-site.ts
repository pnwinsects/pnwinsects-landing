 ...
 *
 * Additive Bunny Storage Zone uploader for the built `_site` directory.
 * Adapted from the pnwmoths uploader of the same name.
 *
 * Walks the local `_site` build output and PUTs files to the ROOT of the bunny
 * `pnwinsects` Storage Zone. The custom domain https://pnwinsects.org/ is the
 * pull zone in front of this bucket.
 *
 * ADDITIVE-ONLY: the script only ever PUTs (creates/overwrites) objects. It
 * never issues a removal request and never runs a destructive sync. There is
 * intentionally no removal helper and no purge step. This invariant is locked
 * by a source-introspection test in upload-site.test.ts that forbids the
 * relevant tokens.
 *
 * Unlike the pnwmoths zone — where the invariant is load-bearing because the
 * species images share the bucket — this zone currently holds only site files.
 * The invariant is kept anyway: it costs nothing, and it means the two repos'
 * uploaders stay reviewable against each other rather than quietly diverging.
 *
 ...
 * pnwinsects Storage Zone → FTP & API Access → Password. Never commit, log,
 * or hardcode it.
 */
...
const BUNNY_ZONE: string = process.env['BUNNY_ZONE'] ?? 'pnwinsects';
...
async function main(): Promise<void> {
  ...
  if (!BUNNY_STORAGE_PASSWORD) {
    ...
    console.error('[upload-site] bunny.net dashboard → Storage → pnwinsects zone → FTP & API Access → Password');
