...
describe('siteObjectStorageUrl', () => {
  it('maps a root file to the zone root with no extra prefix', () => {
    assert.equal(
      ...
      'https://la.storage.bunnycdn.com/pnwinsects/index.html'
    );
  });
  ...
  it('encodes each path segment but preserves "/" separators', () => {
    assert.equal(
      ...
      'https://la.storage.bunnycdn.com/pnwinsects/species/acronicta-americana/records.parquet'
    );
  });
  ...
  it('encodes spaces in a segment as %20 without encoding slashes', () => {
    assert.equal(
      ...
      'https://la.storage.bunnycdn.com/pnwinsects/plates/some%20plate/thumb.html'
    );
  });
});
...
describe('listSiteFiles', () => {
  it('returns POSIX-relative paths for every file recursively, no dirs, no leading ./', () => {
    const dir = mkdtempSync(join(tmpdir(), 'pnwinsects-upload-site-test-'));
...
describe('DRY_RUN smoke', () => {
  it('prints the PUT plan for a fixture and makes zero curl/network calls', () => {
    const siteDir = mkdtempSync(join(tmpdir(), 'pnwinsects-upload-site-dry-'));
    writeFileSync(join(siteDir, 'index.html'), '<html></html>');

    // fileURLToPath, not URL.pathname: on Windows the latter yields "/C:/..."
    // which node then resolves relative to the drive root.
    const scriptPath = fileURLToPath(new URL('./upload-site.ts', import.meta.url));
    const out = execFileSync('node', [scriptPath], {
      env: { ...process.env, DRY_RUN: '1', SITE_DIR: siteDir, BUNNY_STORAGE_PASSWORD: '' },
      encoding: 'utf8',
    });

    assert.match(out, /la\.storage\.bunnycdn\.com\/pnwinsects\/index\.html/);
  });
});
