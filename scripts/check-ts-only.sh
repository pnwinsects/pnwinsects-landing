...
# Permanent TS-only invariant guard, carried over from the pnwmoths repo so the
# two stay diffable. Fails if any of the four regression patterns reappear.
set -e
FAIL=0

# Guard 1: No .js source files in source areas, or at the root eleventy.config.
JS_FILES=$( { find scripts src -name "*.js" 2>/dev/null; ls eleventy.config.js eleventy.config.cjs eleventy.config.mjs 2>/dev/null; } || true )
JS_COUNT=$(printf '%s\n' "$JS_FILES" | grep -c . || true)
if [ "$JS_COUNT" -gt 0 ]; then
  echo "FAIL: $JS_COUNT .js source file(s) found:"
  printf '%s\n' "$JS_FILES"
  FAIL=1
fi
